...
for(v in vars){
  ...
  # v1l1 <- paste0(n, "_impl_lag1")
  # v1l2 <- paste0(n, "_impl_lag2")
  # v1l3 <- paste0(n, "_impl_lag3")
  # v1l4 <- paste0(n, "_impl_lag4")
  # v1l5 <- paste0(n, "_impl_lag5")
  # v1l6 <- paste0(n, "_impl_lag6")
  # 
  # corona_sample.df[, v1l1] <- ave(corona_sample.df[, v1],
  #                                 corona_sample.df$iso_code,
  #                                 FUN = function(x) dplyr::lag(x, 7, default = 0))
  # corona_sample.df[, v1l2] <- ave(corona_sample.df[, v1],
  #                                 corona_sample.df$iso_code,
  #                                 FUN = function(x) dplyr::lag(x, 14, default = 0))
  # corona_sample.df[, v1l3] <- ave(corona_sample.df[, v1],
  #                                 corona_sample.df$iso_code,
  #                                 FUN = function(x) dplyr::lag(x, 21, default = 0))
  # corona_sample.df[, v1l4] <- ave(corona_sample.df[, v1],
  #                                 corona_sample.df$iso_code,
  #                                 FUN = function(x) dplyr::lag(x, 28, default = 0))
  # corona_sample.df[, v1l5] <- ave(corona_sample.df[, v1],
  #                                 corona_sample.df$iso_code,
  #                                 FUN = function(x) dplyr::lag(x, 35, default = 0))
  # corona_sample.df[, v1l6] <- ave(corona_sample.df[, v1],
  #                                 corona_sample.df$iso_code,
  #                                 FUN = function(x) dplyr::lag(x, 42, default = 0))
...
depvars <- c("deaths_s_sm", "cases_s_sm")
  ...
  for(dv in depvars){
    ...
    for(v in vars){
      ...
      if(s == "a"){
        ...
        formula <- paste0(dv, " ~ ", v1, " + ", paste(vlist2[-k], collapse = " + "), " + t2m + t2m_sq + tcc + tp + q + vaccinations_c_imp")
      }
    ...
    save(synthcont.list2, file = paste0(s, "SynthControl_daily_", dvn, "_cut.RData"))
    ...
    p2 <- ggplot(coef.df, aes(date, ATT))+
      geom_ribbon(data = rib.df, aes(x = date, ymin = ribmin, ymax = ribmax, fill = ntreated, group = tmp),
                  alpha = 1, na.rm = TRUE, show.legend = TRUE) +
      scale_fill_manual(values = cols_rib, drop = FALSE) +
      theme(legend.position = "right", text = element_text( size = 14),
            legend.background = element_blank(),
            legend.box.background = element_rect(colour = "black")) +
      guides(fill = guide_legend(title = "N treated", reverse=T))
    ...
    plot.list1[[11]] <- textGrob(paste0(""))
    plot.list1[[12]] <- mylegend
    
    ### Export
    pdf(file = paste0("../03_Output/", s, "Synthetic_daily_", dvn, "_cut", ".pdf"), width = 14, height = 12)
    par(mar = c(0, 0, 0, 0))
    par(mfrow = c(1, 1), oma = c(0, 0, 0, 0))
    grid.arrange(grobs = plot.list1, ncol = 3,
                 left = textGrob(paste0("Change in COVID-19 ", dvn, " / mio capita"), rot = 90, vjust = 1, gp = gpar(fontsize = 18)),
                 bottom = textGrob("Days since intervention", hjust = 0.5, vjust = -0, gp = gpar(fontsize = 18)))
    dev.off()
    ### Export
    png(file = paste0("../03_Output/", s, "Synthetic_daily_", dvn, "_cut", ".png"), width = 14, height = 12,
        units = "in", res = 300)
    par(mar = c(0, 0, 0, 0))
    par(mfrow = c(1, 1), oma = c(0, 0, 0, 0))
    grid.arrange(grobs = plot.list1, ncol = 3,
...
# 
# ####-------------------------------------####
# #### Compute total amount of saved lives ####
# ####-------------------------------------####
# 
# 
# 
# ########################################
# #### Synthethic control second wave ####
# ########################################
# 
# load(paste0("b", "SynthControl_daily_", "deaths", "_cut.RData"))
# 
# 
# vars <- c("c1",
#           "c2",
...
#   
#   # Dataframe
#   coef.df <- data.frame(sc.tmp$est.att[which(rownames(sc.tmp$est.att) %in% c(-34:91)), ])
#   coef.df$date <- as.numeric(rownames(coef.df)) -1
#   
#   # Pretreatment trend
#   if(nrow(coef.df[coef.df$date < 0, ]) == 0){ # NUll model if synth coefs are NA
#     tmp <- lm(rep(0, 35) ~ 1 +  rnorm(35, 0, 0.0001))
#   }else{
#     tmp <- lm(ATT ~ date, coef.df[coef.df$date < 0, ])
#   }
#   
#   # Difference from predicted pre-treatment
#   coef.df <- coef.df[coef.df$date >= 30, ]
#   coef.df$pred <- predict(tmp, newdata = coef.df)
#   coef.df[, "ATT"] <- coef.df[, "ATT"] - coef.df$pred
#   coef.df[, "CI.lower"] <- coef.df[, "CI.lower"] - coef.df$pred
#   coef.df[, "CI.upper"] <- coef.df[, "CI.upper"] - coef.df$pred
#   
#   
#   # Take sums
#   coef.df <- aggregate(coef.df[, c("ATT", "CI.lower", "CI.upper")],
#                        by = list(measure = rep(name, nrow(coef.df))),
#                        FUN = function(x) sum(x))
#   
#   # Make df
#   coef.df$model <- "Since July 2020"
#   if(v == vars[1]){
#     total_eff.df <- coef.df
#   }else{
#     total_eff.df <- rbind(total_eff.df, coef.df)
#   }
#   
#   
#   
...
# ##############
# 
# total_eff.df2 <- total_eff.df
# total_eff.df2[sapply(total_eff.df2, is.numeric)] <- total_eff.df[sapply(total_eff.df2, is.numeric)]  * -1
# 
# # # Order
# # order <- total_eff.df[total_eff.df$model == "Since July 2020", ]
# # order <- order[order(order$ATT),]
# # total_eff.df2$measure <- factor(total_eff.df2$measure, levels = rev(order$measure))
# 
# cols <- gg_color_hue(1)
# 
# 
# # Plot
# zp1 <- ggplot(total_eff.df2, aes(colour = model, shape = model))
# zp1 <- zp1 + geom_hline(yintercept = 0, colour = gray(1/2), lty = 2)
# zp1 <- zp1 + geom_pointrange(aes(x = measure, y = ATT, ymin = CI.lower,
#                                  ymax = CI.upper),
#                              lwd = 1, position = position_dodge2(width = 1/2, reverse = TRUE))
# zp1 <- zp1 + coord_flip() + theme_bw()
# zp1 <- zp1 + theme(legend.title = element_blank())
# zp1 <- zp1 + labs(y = "Lives per mio capita saved within 90 days \n compared to linear pre-treatment trend", x = "")
# zp1 <- zp1 + scale_shape_manual(values = rev(c(19, 17)))
# zp1 <- zp1 + scale_color_manual(values = c(cols))
# zp1 <- zp1 + theme(text = element_text( size = 18),
...
# zp1 <- zp1 + guides(colour = guide_legend(override.aes = list(linetype = 0), reverse = FALSE),
...
# 
# ### Export
# pdf(file = paste0("../03_Output/", "Lives_saved_conf", ".pdf"), width = 12, height = 9)
# par(mar = c(0, 0, 0, 0))
# par(mfrow = c(1, 1), oma = c(0, 0, 0, 0))
# zp1
# dev.off()
...
# 
# ####-----------------------------------------------------------------####
# #### Compute total amount of saved lives according to bootstrap runs ####
# ####-----------------------------------------------------------------####
# 
# 
# 
...
#   }
#   
#   # Dataframe bootstrap runs
#   boot.df <- sc.tmp$att.boot
#   boot.df <- data.frame(boot.df[which(rownames(boot.df) %in% c(31:91)), ])
#   boot.df$date <- as.numeric(rownames(boot.df)) -1
#   
#   
#   # Difference from predicted pre-treatment
#   coef.df <- coef.df[coef.df$date >= 0, ]
#   coef.df$pred <- predict(tmp, newdata = coef.df)
#   coef.df[, "ATT"] <- coef.df[, "ATT"] - coef.df$pred
#   coef.df[, "CI.lower"] <- coef.df[, "CI.lower"] - coef.df$pred
#   coef.df[, "CI.upper"] <- coef.df[, "CI.upper"] - coef.df$pred
#   
#   boot.df[, grepl("boot", names(boot.df))] <- apply(boot.df[, grepl("boot", names(boot.df))], 2,
#                                                     FUN = function(x) x - coef.df$pred)
#   
#   
#   # Take sums
#   coef.df <- aggregate(coef.df[, c("ATT", "CI.lower", "CI.upper")],
...
#                        FUN = function(x) sum(x))
#   
#   boot.df <- aggregate(boot.df[,grepl("boot", names(boot.df))],
#                        by = list(measure = rep(name, nrow(boot.df))),
#                        FUN = function(x) sum(x))
#   boot.df <- reshape(boot.df, varying = names(boot.df)[grepl("boot", names(boot.df))],
#                      direction = "long", v.names = "boot")
#   
#   # Make df
#   coef.df$model <- "Bootstrap distribution with median"
#   if(v == vars[1]){
#     total_eff.df <- coef.df
#   }else{
#     total_eff.df <- rbind(total_eff.df, coef.df)
...
#   
#   # Make df
#   boot.df$model <- "Bootstrap distribution with median"
#   if(v == vars[1]){
#     total_boot.df <- boot.df
#   }else{
#     total_boot.df <- rbind(total_boot.df, boot.df)
#   }
#   
#   
# }
...
# total_eff.df2 <- total_eff.df
# total_eff.df2[sapply(total_eff.df2, is.numeric)] <- total_eff.df[sapply(total_eff.df2, is.numeric)]  * -1
# # total_eff.df2$model <- factor(total_eff.df2$model, levels = c("Since July 2020", "All periods combined"))
# 
# total_boot.df2 <- total_boot.df
# total_boot.df2$boot <- total_boot.df$boot   * -1
# # total_boot.df2$model <- factor(total_boot.df2$model, levels = c("Since July 2020", "All periods combined"))
# 
# 
# 
# # # Order
# # order <- total_eff.df[total_eff.df$model == "All periods combined", ]
# order <- unique(total_eff.df$measure)
# total_eff.df2$measure <- factor(total_eff.df2$measure, levels = rev(order))
# total_boot.df2$measure <- factor(total_boot.df2$measure, levels = rev(order))
# 
# cols <- gg_color_hue(1)
# cols2 <- gg_color_hue(2)[2]
# 
# p1 <- ggplot(total_boot.df2, aes()) +
#   geom_violin(data = total_boot.df2, mapping = aes(x = boot, y = measure, fill = model, col = model), 
#               trim = TRUE, adjust = .5,
#               scale = "area", lwd = 0.8,
#               alpha = .6, position = position_dodge(0.8),
#               draw_quantiles = c(0.5))  +
#   xlab("Lives per mio capita saved within days 30 to 90 after implementation \n compared to linear pre-treatment trend") +
#   ylab("")
# p1 <- p1 + geom_vline(xintercept = 0, colour = gray(1/2), lty = 2)
# p1 <- p1 + geom_errorbar(data = total_eff.df2,
#                          mapping = aes(x = NULL, y = measure, xmin = CI.lower, xmax = CI.upper, shape = "ATT"),
#                          width = 0.2, position =  position_dodge(0.8), 
#                          col = cols2, lwd = 1.1,
#                          show.legend = FALSE)
# p1 <- p1 + geom_pointrange(data = total_eff.df2,
#                            mapping = aes(y = measure, x = ATT, xmin = CI.lower, xmax = CI.upper, shape = "ATT"),
#                            size = 0.8, position =  position_dodge(0.8), 
#                            fill = alpha("white", 0.8), col = cols2,
#                            show.legend = TRUE)
# p1 <- p1 + scale_fill_discrete(name = "model",
#                                labels = c("Bootstrap distribution with median"))
# p1 <- p1 + scale_color_manual(name = "model",
#                               values = darken(cols, 0.3),
#                               labels = c("Bootstrap distribution with median"))
# p1 <- p1 + scale_shape_manual(name = "ATT",
#                               values = c(21),  labels = c("ATT with 95% CI"))
# p1 <- p1 + theme_bw()
# p1 <- p1 + theme(legend.title = element_blank())
# p1 <- p1 + theme(text = element_text( size = 20),
#                  legend.position = c(0.99, 0.01), legend.justification = c(0.99, 0.01),
#                  legend.key = element_blank(),
#                  legend.margin = margin(-0.2,0.05,0,0.05, unit = "cm"),
#                  legend.background = element_blank(),
#                  legend.box.background = element_rect(colour = "black"),
#                  axis.text.y = element_text(colour = "black"),
#                  plot.title = element_text(hjust = 0.5),
...
#                  axis.text.x = element_text(colour = "black")
# )
# # p1 <- p1 + coord_cartesian(xlim = c(-230, 250)) 
# p1 <- p1 + ggtitle(element_blank())
# p1 <- p1 + guides(colour = guide_legend(override.aes = list(shape = 0, col = darken(cols, 0.3), 
#                                                             size = 1.3, lwd = 1), reverse = TRUE),
#                   shape = guide_legend(override.aes = list(linetype = 0), reverse = FALSE))
# 
# p1
# 
# 
...
# 
# ### Export
# pdf(file = paste0("../03_Output/", "Lives_saved_violin", ".pdf"), width = 13, height = 10)
# par(mar = c(0, 0, 0, 0))
# par(mfrow = c(1, 1), oma = c(0, 100, 0, 0))
# p1
# dev.off()
# 
# ### Export
# png(file = paste0("../03_Output/", "Lives_saved_violin", ".png"), width = 13, height = 10,
#     units = "in", res = 300)
# par(mar = c(0, 0, 0, 0))
# par(mfrow = c(1, 1), oma = c(0, 100, 0, 0))
# p1
...
# 
# ####-----------------------------------------------------------------####
# #### Compute total amount of saved cases according to bootstrap runs ####
# ####-----------------------------------------------------------------####
# 
# 
# 
...
# ########################################
# 
# load(paste0("b", "SynthControl_daily_", "cases", "_cut.RData"))
# 
# 
# vars <- c("c1",
#           "c2",
...
#               alpha = .6, position = position_dodge(0.8),
#               draw_quantiles = c(0.5))  +
#   xlab("Cases per mio capita prevented within days 30 to 90 after implementation \n compared to linear pre-treatment trend") +
#   ylab("")
# p1 <- p1 + geom_vline(xintercept = 0, colour = gray(1/2), lty = 2)
# p1 <- p1 + geom_errorbar(data = total_eff.df2,
#                          mapping = aes(x = NULL, y = measure, xmin = CI.lower, xmax = CI.upper, shape = "ATT"),
...
# p1 <- p1 + theme(legend.title = element_blank())
# p1 <- p1 + theme(text = element_text( size = 20),
#                  legend.position = c(0.01, 0.90), legend.justification = c(0.01, 0.90),
#                  legend.key = element_blank(),
#                  legend.margin = margin(-0.2,0.05,0,0.05, unit = "cm"),
#                  legend.background = element_blank(),
#                  legend.box.background = element_rect(colour = "black"),
...
#                  axis.text.x = element_text(colour = "black")
# )
# p1 <- p1 + coord_cartesian(xlim = c(-10000, 12000))
# p1 <- p1 + ggtitle(element_blank())
# p1 <- p1 + guides(colour = guide_legend(override.aes = list(shape = 0, col = darken(cols, 0.3), 
#                                                             size = 1.3, lwd = 1), reverse = TRUE),
#                   shape = guide_legend(override.aes = list(linetype = 0), reverse = FALSE))
...
# 
# ### Export
# pdf(file = paste0("../03_Output/", "Cases_saved_violin", ".pdf"), width = 13, height = 10)
# par(mar = c(0, 0, 0, 0))
# par(mfrow = c(1, 1), oma = c(0, 100, 0, 0))
# p1
# dev.off()
# 
# ### Export
# png(file = paste0("../03_Output/", "Cases_saved_violin", ".png"), width = 13, height = 10,
#     units = "in", res = 300)
# par(mar = c(0, 0, 0, 0))
# par(mfrow = c(1, 1), oma = c(0, 100, 0, 0))
# p1
